...
public class StreamsExample {
    ...
    public static void main(final String[] args) {
        ...
        Predicate<Author> activeAuthorPrintPerdicate = new Predicate<Author>() {
            @Override
            public boolean test(Author author) {
                return author.active;
            }
        };
        authors.stream().filter(activeAuthorPrintPerdicate).forEach(System.out::println);
        ...
        Predicate<Book> activeBooksAuthorsPrintPerdicate = new Predicate<Book>() {
            @Override
        ...
        authors
                ...
                .flatMap(new Function<Author, Stream<Book>>() {
                    @Override
                    public java.util.stream.Stream<Book> apply(Author author) {
                        return author.books.stream();
                    }
                })
                .filter(activeBooksAuthorsPrintPerdicate)
        ...
        // SOLVED With functional interfaces declared
        Function<Book, Integer> bookPriceFunction = new Function<Book, Integer>() {
            @Override
        ...
        double avgPrice = authors
                .stream()
                .flatMap(new Function<Author, java.util.stream.Stream<Book>>() {
                    @Override
                    public java.util.stream.Stream<Book> apply(Author author) {
                        return author.books.stream();
                    }
                })
                .mapToInt(new ToIntFunction<Book>() {
                    @Override
        ...
        // SOLVED With functional interfaces declared
        Predicate<Author> activeWithPublishedBookPredicate = new Predicate<Author>() {
            @Override
            public boolean test(Author author) {
        ...
        authors
                .stream()
                .filter(activeWithPublishedBookPredicate)
                .forEach(System.out::println);
